...
export default function ShoppingTab({ items, onItemsChange }: Props) {
  ...
  const markAllOk = () => {
    ...
    setChecked(new Set());
    toast.success('Alle als "Gut" markiert!');
  };

  const copyToClipboard = () => {
    if (shoppingItems.length === 0) return toast.error('Die Einkaufsliste ist leer.');
    const lines: string[] = [];
    if (emptyItems.length > 0) {
      lines.push('🛒 Fehlt komplett:');
      emptyItems.forEach((i) => lines.push(`  • ${i.name} (${i.quantity})`));
    }
    if (lowItems.length > 0) {
      lines.push('⚠️ Wenig vorhanden:');
  ...
  if (shoppingItems.length === 0) {
    return (
      <div className="p-4 flex flex-col items-center justify-center min-h-[60vh] text-center space-y-3">
        ...
        <p className="text-white font-semibold text-lg">Alles gut!</p>
        <p className="text-slate-400 text-sm">Dein Kühlschrank ist ausreichend befüllt.</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 space-y-5 max-w-lg mx-auto">
      ...
      <div className="flex gap-2">
        ...
        >
          <CheckSquare size={15} /> Alle erledigt
        </button>
      </div>
      ...
      {emptyItems.length > 0 && (
        <div className="space-y-2">
          <p className="text-red-400 font-semibold text-xs uppercase tracking-widest px-1">
            Fehlt / Leer
          </p>
          <div className="space-y-2">
            {emptyItems.map((item) => (
      ...
      {lowItems.length > 0 && (
        <div className="space-y-2">
          <p className="text-amber-400 font-semibold text-xs uppercase tracking-widest px-1">
            Wenig vorhanden
          </p>
          <div className="space-y-2">
            {lowItems.map((item) => (
...
function ShoppingItem({ item, checked, onToggle, variant }: ShoppingItemProps) {
  const borderColor = variant === 'empty' ? 'border-red-500/30' : 'border-amber-500/30';
  return (
    <button
      onClick={onToggle}
      className={`w-full bg-slate-800 border ${borderColor} rounded-2xl px-4 py-3 flex items-center gap-3 transition hover:bg-slate-700 text-left ${checked ? 'opacity-50' : ''}`}
    >
      <div
      ...
      <div className="flex-1 min-w-0">
        <p className={`text-white text-sm font-medium ${checked ? 'line-through' : ''}`}>
          {item.name}
        </p>
